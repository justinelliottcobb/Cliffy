...
impl HookManager {
    ...
    pub fn use_geometric_state(&self, initial_value: Multivector3D<f64>) -> (GeometricState, impl Fn(Multivector3D<f64>)) {
        let hook_id = self.next_hook_id();
        let mut state_hooks = self.state_hooks.lock().unwrap();

        let behavior = state_hooks.entry(hook_id)
        ...
        F: Fn() + Send + Sync + 'static
    {
        ...
        if should_run {
            ...
            // Store the new hook
            effect_hooks.insert(hook_id, EffectHook {
                dependencies,
                cleanup: None,
    ...
    pub fn use_effect_with_cleanup<F, C>(&self, effect: F, cleanup: C, dependencies: Vec<Multivector3D<f64>>)
    where
        F: Fn() + Send + Sync + 'static,
    ...
    pub fn use_memo<F>(&self, computation: F, dependencies: Vec<Multivector3D<f64>>) -> Multivector3D<f64>
    where
        F: Fn() -> Multivector3D<f64>
    {
        ...
        if should_recompute {
            ...
            memo_hooks.insert(hook_id, MemoHook {
                dependencies,
    ...
    pub fn use_geometric_transform(&self, initial_transform: Multivector3D<f64>) -> (GeometricState, impl Fn(Multivector3D<f64>)) {
    ...
    pub fn use_derived_behavior<F>(&self, dependencies: Vec<GeometricState>, computation: F) -> GeometricState
    where
        F: Fn(&[Multivector3D<f64>]) -> Multivector3D<f64> + 'static
    {
        ...
        for (i, dependency) in dependencies.iter().enumerate() {
            ...
            let _unsubscribe = dependency.subscribe(move |_| {
                let current_values: Vec<_> = dependencies_clone.iter().map(|b| b.sample()).collect();
